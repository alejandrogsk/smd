...
const ServicesSection = ({services}) => {
  ...
  return (
    <section className='py-20 services-section'>
      ...
      <h2 className='text-white text-3xl md:text-5xl	font-semibold text-start	'>{title}</h2>
        ...
        <div className='services-section__grid my-16'>
          {
            services_list.map((item, i) => (
              <div key={i} className='services-section__grid-item cursor-pointer'>
                ...
                <h3 className='text-white my-4 font-semibold text-4xl	'>{item.title}</h3>

                <a className='text-white text-xl capitalize underline hover:text-ui-orange-main'>{item.link}</a>
              </div>
            ))
          }
        </div>
